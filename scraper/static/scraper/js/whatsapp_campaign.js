// ==========================================
// WhatsApp Campaign Page JavaScript
// ==========================================
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('confirmWhatsApp').addEventListener('change', function(e) {
...
function initializeForm() {
    const form = document.getElementById('whatsappCampaignForm');
...
async function loadPendingContacts() {
    ...
    try {
        ...
        document.getElementById('pendingWhatsAppCount').textContent = '-';
...
async function handleFormSubmit(e) {
    ...
    if (!document.getElementById('confirmWhatsApp').checked) {
    ...
    try {
        const response = await API.post('/startWhatsappCampaign', {});
        ...
        Toast.success('WhatsApp campaign started successfully!');
        ...
        // Hide form
        document.getElementById('whatsappCampaignForm').style.display = 'none';
        
        // Start monitoring
        startStatusMonitoring(response.task_id);
        
    } catch (error) {
        Toast.error(`Failed to start WhatsApp campaign: ${error.message}`);
        submitBtn.disabled = false;
        submitBtn.innerHTML = '<span class="btn-icon"><i class="fas fa-rocket"></i></span> Start WhatsApp Campaign';
    }
}
...
async function checkTaskStatus(taskId) {
    try {
        ...
        if (status.status === 'completed') {
            clearInterval(statusCheckInterval);
            Toast.success('WhatsApp campaign completed successfully!');
            ...
            clearInterval(statusCheckInterval);
            Toast.error('WhatsApp campaign failed: ' + (status.error || 'Unknown error'));
